...
//    logic mydir = 0; //direction of the servo, 0 is left, 1 is right
    always_ff @(posedge clk) begin
        if (servo_stop == 1'b0) begin
            if (hz_count == 28'd50_000_000) begin
                hz_count <= 28'b0;
               
//                if (myang >= 8'd250) mydir <= 0;
//                else if (myang <= 8'd5) mydir <= 1;
               
                if (servo_dir == 1'b0 && myang > 8'd5) myang <= myang-28'd5;
                else if (servo_dir == 1'b1 && myang < 8'd250) myang <= myang +28'd5;
            end else begin
